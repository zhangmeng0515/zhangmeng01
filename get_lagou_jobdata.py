...
def get_a_page(result):  # 获取一页的信息
    field = ['companyFullName','companyShortName','companyShortName','companySize','financeStage',
             'district','positionName','workYear','education','salary','positionAdvantage']  # 要获取哪些字段信息
    page_info = []
    try:
        for i in result:
            info = []
...
def main():
    ...
    num=get_pages(total_count/15)  #每页15个职位，总职位数300多个，比较少
    total_info=[]
